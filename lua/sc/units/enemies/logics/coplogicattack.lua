...
--[[function CopLogicBase.on_importance(data)
	local imp_state = data.important
	local my_data = data.internal_data
	local detection_id = my_data.detection_task_key
	local update_func_id = my_data.update_queue_id
	...
	if detection_id then
		if imp_state then
			local task_data = e_manager._queued_tasks[detection_id]

			if task_data and not task_data.asap then
				task_data.previous_imp_t = task_data.t

				local t = TimerManager:game():time()

				update_task(e_manager, detection_id, nil, nil, t, nil, true)
			end
		else
			local task_data = e_manager._queued_tasks[detection_id]

			if task_data and not task_data.previous_imp_t then
				update_task(e_manager, detection_id, nil, nil, task_data.previous_imp_t, false)
				task_data.previous_imp_t = nil
			end
		end
	end
			...
			if best_cover and not my_data.processing_cover_path and not my_data.cover_path and not my_data.charge_path_search_id then
				...
				if not in_cover or best_cover[1] ~= in_cover[1] then
					...
					if unobstructed_line then
						...
						--[[local line = Draw:brush(Color.blue:with_alpha(0.5), 5)
						line:cylinder(my_pos, to_cover_pos, 25)]]
					else
						data.brain:add_pos_rsrv("path", {
							radius = 60,
...
function CopLogicAttack.is_available_for_assignment(data, new_objective)
	...
	if new_objective then
		local allow_trans, obj_fail = CopLogicBase.is_obstructed(data, new_objective, 0.2, att_obj, true)
...
function CopLogicAttack._chk_wants_to_take_cover(data, my_data)
	...
	if my_data.moving_to_cover or data.is_suppressed or my_data.attitude ~= "engage" then
		return true
	elseif data.unit:anim_data().reload then
		return "reload"
	end
...
function CopLogicAttack.queue_update(data, my_data)
	local delay = data.important and 0 or 1
