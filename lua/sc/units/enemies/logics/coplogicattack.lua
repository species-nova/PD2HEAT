...
function CopLogicAttack.enter(data, new_logic_name, enter_params)
	...
	elseif data.unit:base().has_tag then
		if data.unit:base():has_tag("medic_summers") or data.unit:base():has_tag("fbi_vet") or data.unit:base():has_tag("lpf") then
			my_data.use_brain = true
		end
	end
...
function CopLogicAttack._upd_combat_movement(data)
	...
	elseif not enemy_visible_soft or not my_data.stay_out_time or not enemy_visible then
		if in_cover then
...
function CopLogicAttack._upd_aim(data, my_data)
				...
				if time_since_verification then
					...
					if aim and my_data.shooting and REACT_SHOOT <= focus_enemy.reaction then
						if running then
							...
							if same_height and time_since_verification < 1 then
								shoot = true
							end
	...
	if aim or shoot then
		...
		if focus_enemy.verified or focus_enemy.nearly_visible or time_since_verification and time_since_verification <= 0.3 then
			...
			if time_since_verification and time_since_verification <= 10 or focus_enemy.dis < 400 then
				look_pos = focus_enemy.verified_pos or focus_enemy.last_verified_pos or my_data.expected_pos
			end
