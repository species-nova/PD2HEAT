...
function ContourExt:add(type, sync, multiplier, override_color, add_as_child)
	...
	if sync then
		if add_as_child then
			...
			--for this to work, the receiver parameter for sync_contour_state needs to be changed to "connection"
			--since a -1 id means the unit is not attached to the network (happens to NPCs when outside of stealth)
			--network functions with "unit" as the receiver require a unit to be sent in the message, and it to be network-attached
			if u_id == -1 then
