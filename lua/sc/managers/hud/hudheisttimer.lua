...
function HUDHeistTimer:init(hud, tweak_hud)
	...
	--[[
	self._heist_timer_panel:polyline({
		color=tweak_data.screen_color_blue:with_alpha(0.5), 
		blend_mode="add", 
		line_width=1, 
		closed=true, 
		points={ Vector3( 0, 0, 0 ), Vector3( self._heist_timer_panel:w(), 0, 0 ), Vector3( self._heist_timer_panel:w(), self._heist_timer_panel:h(), 0 ), Vector3( 0, self._heist_timer_panel:h(), 0 ) } 
	})
	]]--
	...
	self._timer_shadow = self._heist_timer_panel:text({
		name = "timer_shadow",
		vertical = "top",
		word_wrap = false,
		wrap = false,
...
function HUDHeistTimer:set_time(time)
	...
	self._timer_shadow:set_text(text)
end
