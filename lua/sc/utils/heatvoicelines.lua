HeatVoiceline = HeatVoiceline or blt_class()
function HeatVoiceline:init()
	blt.xaudio.setup() --Must be called at least once for custom audio to be loaded.
	self._custom_voice_path = Application:nice_path("./" .. ModPath .. "assets/oggs", true)
		...
		if line_type and #line_type > 0 then
			...
			if buffer then
				if not existing_source then
					...
					unit:base():add_destroy_listener("HEATRemoveAudioSource", function()
